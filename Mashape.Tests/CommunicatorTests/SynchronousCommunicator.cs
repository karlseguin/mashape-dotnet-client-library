...
namespace Mashape.Tests.CommunicatorTests
{
   public class SynchronousCommunicator : BaseFixture
   {
      ...
      public void ThrowsExceptionIfNetworkIsntAvailable()
      {
         ...
         var ex = Assert.Throws<MashapeException>(() => Communicator.SendPayload<object>(new RequestContext(HttpMethods.Get, null)));
      ...
      public void SendsMessageToServer()
      {
         ...
         Communicator.SendPayload<object>(new RequestContext(HttpMethods.Get, new Dictionary<string, object>{{"lid", "theid"}, {"scope", 2}}));
      }
      ...
      public void SendsAPostMessageToServer()
      {
         ...
         Communicator.SendPayload<object>(new RequestContext(HttpMethods.Post, new Dictionary<string, object> { { "lid", "theid" }, { "score", 44} }));
      }
      ...
      public void GetsResponseFromServer()
      {
         ...
         var response = Communicator.SendPayload<AResponse>(new RequestContext(HttpMethods.Get, new Dictionary<string, object> { { "id", "1" } }));
      ...
      public void ThrowsExceptionOnDeserializationErrors()
      {
         ...
         var ex = Assert.Throws<MashapeException>(() => Communicator.SendPayload<AResponse>(new RequestContext(HttpMethods.Get, Enumerable.Empty<KeyValuePair<string, object>>())));
